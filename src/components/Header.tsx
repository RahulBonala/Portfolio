...
const Header: React.FC = () => {
  ...
  const onBookingFlow = pathname.startsWith('/teach');
    ...
    <header className={`header ${isScrolled ? 'scrolled' : ''}`}>
      <div className="header-bar">
        ...
        <div className="header-actions">
          {!onBookingFlow && (
            <Link to="/teach" className="header-book">
              Book a session
            </Link>
          )}
          <button
      ...
      <div className={`menu-overlay ${isMenuOpen ? 'open' : ''}`} aria-hidden={!isMenuOpen}>
        {!onBookingFlow && (
          <Link to="/teach" className="menu-overlay-book" onClick={closeMenu} tabIndex={isMenuOpen ? 0 : -1}>
