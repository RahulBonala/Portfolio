...
type Study = {
  ...
  problem: string;
  constraints: string[];
  coreDecision: { decision: string; body: string; rejected: { name: string; why: string }[] };
  loop: string[];
  outcomes: { intro: string; items: Outcome[] };
  redesign: string[];
  liveHref?: string;
};
...
const STUDIES: Record<string, Study> = {
  ...
  'smiths-detection': {
    ...
    outcomes: {
      ...
      items: [
        { metric: '95%', label: 'CSAT across three regions', method: 'From the recurring customer-satisfaction surveys run with the regional service teams.' },
        { metric: '80%', label: 'faster maintenance workflows', method: 'Time to complete the core service task, compared before and after the console rollout.' },
        { metric: '70%', label: 'fewer support tickets', method: 'Drop in console-related support tickets in the period following launch and usability testing.' },
      ],
    },
    redesign: [
    ...
    <article className="case" ref={ref}>
      <div className="container case-narrow">
        ...
        <section className="case-section" aria-labelledby="cs-loop">
          ...
          {study.loop.map((p, i) => <p className="case-body" data-reveal="up" key={i}>{p}</p>)}
        </section>
